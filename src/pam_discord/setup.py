...
def _write_always_on_config(
    ...
    *,
    ...
    whisper_model, whisper_device, whisper_compute_type = _whisper_defaults()
    content = f"""# Generated by `pam setup`.
mode = "central"
archive_dir = {_toml_string(str(state_dir / "archive"))}
instance_lock_dir = {_toml_string(str(state_dir / "instance.lock"))}
allowed_user_ids = [{user_id}]
...
[central]
guild_id = {guild_id}
approved_roots = [{_toml_string(str(approved_root))}]
excluded_roots = []
state_dir = {_toml_string(str(state_dir / "central"))}
index_sessions_per_directory = 10
recent_sessions_limit = 10
sidebar_sessions_per_forum = 0
sidebar_session_max_age_days = 7
"""
    _write_private(path, content)
...
def _setup_identity(args: argparse.Namespace) -> None:
    ...
    if args.identity_only:
        print("\npam identity saved.")
        return
    print("\npam identity saved. Now create your central pam server.")
    project_args = ["--state-dir", str(state_dir)]
    if args.no_service:
        project_args.append("--no-service")
    project_add(project_args, hub=True)
...
def setup(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Configure one central Pam server for an approved projects directory"
    )
    parser.add_argument(
        "project",
        nargs="?",
        type=Path,
        help="legacy shortcut: configure one dedicated project server",
    )
    ...
    print(
        ...
        "Intent. pam will guide you through creating a Discord server and installing "
        "the bot there.\n"
    ...
    *,
    ...
    if config_path.exists():
        ...
        if hub and existing.project_roots:
            raise SystemExit("The central pam server is already configured.")
        if not hub and existing.mode == "central":
            raise SystemExit(
                f"{workspace} is discovered automatically in central mode; "
                "no project-specific server is needed."
            )
    ...
    config_suffix = _hub_config_block(projects_root) if projects_root is not None else ""
    if hub and config_path.exists():
        raise SystemExit(
            "A Pam configuration already exists. Add a central destination manually or "
            "use a fresh state directory."
        )
    if hub:
        assert projects_root is not None
        _write_always_on_config(
            config_path,
            state_dir=state_dir,
            user_id=user_id,
            guild_id=guild_id,
    ...
    if not hub:
        _configure_project_archive_git(workspace, ignore=ignore_history)
    if hub:
        print(f"\nCentral pam server ready for projects under: {projects_root}")
    else:
...
def doctor(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        print(f"OK  deployment mode: {config.mode}")
        print(
            f"OK  workspace mappings: {len(config.guilds)} dedicated server(s), "
    ...
    if token and token != "replace-me":
        ...
        if config and config.always_on_guild_id is not None:
            guild_ids = sorted(set((*guild_ids, config.always_on_guild_id)))
